...
// LogLevel indicates the severity of a logger entry
type LogLevel int
...
// SetLevel sets the global logger level. Only entries with level equals or higher than
// this value will be logged
func SetLevel(logLevel LogLevel) {
	globalLogLevel = logLevel
}

// GetLevel returns current global logger level
...
// this value will be logged. However, this is also capped by the global logger level. That is,
// messages with lower level than global-logger-level will be discarded at any case.
func SetSyslogLevel(logLevel LogLevel) {
	syslogLevel = logLevel
}

// logFormattedEntry nicely formats and emits a logger entry
...
// logEntry emits a formatted logger entry
...
// logErrorEntry emits a logger entry based on given error object
